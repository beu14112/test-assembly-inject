...
Patch của Beu nhắm đến ứng dụng bằng bundle identifier ổn định. Gói patch không lưu UUID container vì UUID này thay đổi theo từng máy và mỗi lần cài lại ứng dụng.

## Cấu trúc workspace

Khi tạo patch tên `ABC` cho bundle `com.abc.xyz`, Beu tạo cây thư mục có thể chỉnh sửa như sau:
...
4. Bấm **Xong**. Beu sẽ tạo dự án và workspace có thể chỉnh sửa.
5. Mở **Tệp → Workspace Beu → Patches → tên patch → bundle ID**.
...
3. Beu tự lấy bundle identifier ổn định và đường dẫn tương đối, sau đó mở bản nháp patch.
...
- Toàn bộ lần ghi có nhật ký và kiểm tra lại dữ liệu. Nếu lỗi giữa chừng, Beu sẽ cố gắng rollback giao dịch.
...
- Có thể nhập từ ứng dụng Tệp bằng cách mở hoặc chia sẻ gói `.3105` sang Beu.
...
- Trên máy hoặc lần cài mới, gói có bảo vệ sẽ hỏi mật khẩu một lần. Beu lưu content key đã mở khóa trong Keychain; file xuất ra vẫn được mã hóa và luôn gắn với mật khẩu ban đầu.
...
- Beu từ chối symbolic link, đường dẫn tuyệt đối, thành phần `..`, bundle không hợp lệ và nhiều mục trỏ đến cùng một đích.
