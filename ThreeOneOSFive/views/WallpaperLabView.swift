...
struct WallpaperLabView: View {
        ...
        Section {
            if let report {
                HStack {
                    ...
                    .foregroundStyle(report.canInstall ? theme.success : theme.warning)
                    Spacer()
                    Text("MHA-C2")
    ...
    private func packageRow(_ package: WallpaperStagedPackage) -> some View {
        HStack(spacing: 12) {
            ...
            .foregroundStyle(
                selectedPackageID == package.id ? accent : Color.secondary
            )
        }
    ...
    private var busyOverlay: some View {
        if isBusy {
            ZStack {
                ...
                .padding(.vertical, 18)
                .background(.regularMaterial, in: RoundedRectangle(cornerRadius: AppTheme.radiusMedium))
            }
        }
    }
