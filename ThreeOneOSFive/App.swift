...
struct BeuApp: App {
    ...
    // Onboarding intro pages removed — always go straight to license / main content.
    ...
    init() {
        setupLogCapture()
        log("app: Beu launching — iOS \(AppInfo.osVersion) (\(AppInfo.osBuild)) \(AppInfo.machineName)")
    }
    ...
    private var activeAccent: Color {
        AppTheme.accent(hue: themeHue)
    }
    private var appTheme: BeuTheme {
        BeuTheme.make(accent: activeAccent)
    }
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                // Global looping video background for entire app
                ...
                if licenseManager.isAuthorized {
                    ContentView()
                        ...
                        .environment(\.beuTheme, appTheme)
                        .environment(\.accentColorTheme, appTheme.accent)
                        .tint(activeAccent)
                        .opacity(showOnboarding ? 0 : 1)
                        .allowsHitTesting(!showOnboarding)
                } else if !showOnboarding {
                    LicenseGateView(licenseManager: licenseManager)
                        .environment(\.appLanguage, language)
                        .environment(\.locale, language.locale)
                        .environment(\.beuTheme, appTheme)
                        .environment(\.accentColorTheme, appTheme.accent)
                ...
                if showOnboarding {
                    OnboardingView {
                        ...
                        Task { await licenseManager.start() }
                    }
                    .environment(\.appLanguage, language)
                    .environment(\.locale, language.locale)
                    .environment(\.beuTheme, appTheme)
            ...
            .animation(nil, value: languageCode)
