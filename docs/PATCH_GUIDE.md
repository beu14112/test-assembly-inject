...
Beu patch packages target an app by its stable bundle identifier. They never store the app-container UUID, because that UUID changes across devices and reinstalls.
...
4. Tap **Done**. Beu creates the project and its editable workspace.
5. Open **Files → Beu Workspace → Patches → project name → bundle ID**.
...
3. Beu captures the stable bundle identifier and relative path automatically and opens a patch draft.
...
- Writes are journaled and verified. If applying fails partway through, Beu attempts to roll the transaction back.
...
- Import from Files by opening or sharing a `.3105` package to Beu.
...
- On a new device or installation, a protected package asks for its password once. Beu stores the unlocked content key in Keychain; the exported package remains encrypted and tied to its original password.
